...
using namespace std;
unordered_map<int, vector<int>> Neighborhood::RD;
unordered_map<int, vector<int>> Neighborhood::PN;
...
bool Neighborhood::feasible_insert(Item& item, Bin& bin)
{
...
Chromosome Neighborhood::feasible_reinsert_tweak(Chromosome offspring)
{
...
Chromosome Neighborhood::feasible_between_swap_tweak(Chromosome offspring)
{
	Chromosome old_offspring = offspring;
	random_device rd;
	mt19937 gen(rd());
...
Chromosome Neighborhood::item_overlap_removal(Chromosome offspring)
{
	random_device rd;
	mt19937 gen(rd());
...
Chromosome Neighborhood::empty_bin_tweak(Chromosome offspring)
{
	random_device rd;
	mt19937 gen(rd());
	vector<Bin>& bins = offspring.get_groups();
	...
	for (auto& [id, item] : bin1.get_items())
	{
...
Chromosome Neighborhood::within_swap_tweak(Chromosome offspring)
{
	return offspring;
}
...
Chromosome Neighborhood::apply_tweak(FunctionID func, Chromosome offspring)
{
	switch (func)
	{
		break;
	case Feasible_Reinsert:
		return feasible_reinsert_tweak(offspring);
		break;
	//case Item_Overlap:
	...
	case Feasible_Between_Swap:
		return feasible_between_swap_tweak(offspring);
		break;
	case Empty_Bin:
		return empty_bin_tweak(offspring);
		break;
	}
	return offspring;
}

FunctionID Neighborhood::select_tweak(vector<double> tweak_probabilities)
{
...
int Neighborhood::select_item(Bin& bin)
{
	random_device rd;
	mt19937 gen(rd());
	unordered_map<int, Item>& bin_items = bin.get_items();
