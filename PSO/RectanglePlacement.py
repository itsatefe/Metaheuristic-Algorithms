...
class Event:
    def __init__(self, x, is_left, rect):
        self.x = x
    ...
    def __lt__(self, other):
        ...
        if self.is_left != other.is_left:
            return self.is_left #and not other.is_left
...
def calculate_unoccupied_y(tree, container_height):
    intervals = tree.list_intervals()

    # Calculate unoccupied areas
    unoccupied = []
    last_y = 0
    sorted_intervals = sorted(intervals)  # Ensure intervals are sorted

    for start, end in sorted_intervals:
        if last_y < start:
            unoccupied.append((last_y, start))
...
def detect_unoccupied_areas(rectangles, container_width, container_height):
    events = []
    active_intervals = IntervalTree()

    # Create events for each rectangle
    for  rect in rectangles:
        events.append(Event(rect.x, True, rect))
        events.append(Event(rect.x + rect.get_width(), False, rect))
    ...
    for event in events:
        ...
        if x != last_x:
            ...
            for uy_start, uy_end in unoccupied_y:
                unoccupied_spaces.append(Space(last_x, x, uy_start, uy_end))
            last_x = x

        if event.is_left:
            active_intervals.insert_interval(Interval(event.rect.y, event.rect.y + event.rect.get_height(), event.rect))
        else:
            active_intervals.remove_interval(Interval(event.rect.y, event.rect.y + event.rect.get_height(), event.rect))

    # Final sweep to the end of the container width
    unoccupied_y = calculate_unoccupied_y(active_intervals, container_height)
    for uy_start, uy_end in unoccupied_y:
        unoccupied_spaces.append(Space(last_x, container_width, uy_start, uy_end))
